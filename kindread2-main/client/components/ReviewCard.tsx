...
function StarRating({ rating }: { rating: number }) {
  return (
    <View style={starStyles.container}>
      {[1, 2, 3, 4, 5].map((i) => (
        <ThemedText key={i} style={[starStyles.star, { color: i <= rating ? COLORS.amber : "#E5E5EA" }]}>
          {i <= rating ? "\u2605" : "\u2605"}
        </ThemedText>
      ))}
    </View>
...
export function ReviewCard({ bookmark, onPress, onCommentPress, onRefresh, userStreakDays = 0 }: ReviewCardProps) {
  ...
  const bookMetaLabel = (() => {
    const parts: string[] = [];
    if (readingDays !== null && readingDays > 1) parts.push(`${readingDays} days`);
    if (bookmark.book?.totalPages) parts.push(`${bookmark.book.totalPages}p`);
    ...
    >
      ...
      {hasTextContent ? (
        <View style={styles.noteContainer}>
          <ThemedText style={[FeedTypography.body, { color: theme.text }]} numberOfLines={6}>
      ...
      {bookmark.book ? (
        <View style={styles.bookRow}>
          ...
          <View style={styles.bookInfo}>
            ...
            {bookMetaLabel ? <ThemedText style={[FeedTypography.bookMeta, { color: theme.textTertiary }]}>{bookMetaLabel}</ThemedText> : null}
          </View>
        </View>
      ) : null}
